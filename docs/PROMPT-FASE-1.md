...
1. cd "C:\Users\torme\OneDrive\Desktop\FWD Contable AI\engine" ; npm install
2. Copy-Item .env.example .env
3. npm run db:init
   (crea engine/data/fwd-contable.db con todas las tablas, vistas e inserta FUNDACION CRC Endurance)
4. npm run dev
...
- Los skills en engine/skills/*.md son sagrados. NO los reescribas. Solo los cargás como contexto del sub-agente.
...
Crear:
  ...
  engine/src/lib/pdf-extract.ts
  engine/src/lib/xml-parse.ts
  engine/src/lib/validator.ts
  engine/src/lib/db-init.ts
  engine/src/agents/claude.ts
  engine/src/agents/docscan.ts
  engine/src/agents/tax-iva.ts
  engine/src/routes/health.ts
  engine/src/routes/process-document.ts
  engine/src/types/factura.ts
  engine/tests/docscan.test.ts
  engine/tests/reconciliation.test.ts
  engine/data/.gitkeep
  engine/data/uploads/.gitkeep
...
- db.ts: abre engine/data/fwd-contable.db con better-sqlite3 con `WAL` y `foreign_keys=ON`. Exporta `db` singleton.
- db-init.ts: lee engine/schemas/db.sql y lo ejecuta contra db. Script invocable con `npm run db:init`.
- Agregá script `"db:init": "tsx src/lib/db-init.ts"` en package.json.

PASO 5 — validator.ts
- Cargá engine/schemas/factura.schema.json con ajv (con ajv-formats para validar `date`).
...
- Exportá una función `createSubagent({ name, skillPath, model? })` que:
  - Lee el archivo de skill (e.g. engine/skills/docscan.md) como string.
...
- Importa createSubagent y crea `docscanAgent` con skill `engine/skills/docscan.md`.
...
- Importa createSubagent y crea `taxIvaAgent` con skill `engine/skills/tax-iva.md`.
...
- Guardá el archivo en engine/data/uploads/<uuid>-<filename>.
...
- Habilitá multer con destino engine/data/uploads/, límite 10MB por archivo.
...
- Vitest.
- 4 test cases, uno por PDF golden en engine/tests/fixtures/:
...
- NO subir archivos a engine/data/ al repo (ya está en .gitignore).
